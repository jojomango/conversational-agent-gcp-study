...
# 
# [COMMENTED OUT - D17+]
# 原因：專案改用 CX Agent Studio（托管服務），知識庫由 GCP 管理。
# 保留此檔案作為未來實作「自建 RAG (方案 B)」時的參考（D29-D30 對比實驗）。
# 
# 要重新啟用：取消下方註解並確保 secrets.tf 的 db-password 有建立 version。
# 費用：db-f1-micro ≈ $10/月
...
#   name     = "bff_user"
#   instance = google_sql_database_instance.postgres_instance.name
#   # D17: 密碼改從 Secret Manager 讀取，避免明碼進 Git
